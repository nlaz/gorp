...
//!
//! `GORP_TRACE_FILE=<path>` appends a line per invocation and is the primary
...
use gorp_core::search::{SearchOptions, SearchResult};
...
/// The envelope's schema tag, and the contract between this binary and the
/// eval harnesses that read `GORP_TRACE_FILE`.
///
/// Bumped from `semgrep.trace/1` by the rename, with the same field set —
/// the wire shape did not change, only the name. Recorded runs under
/// `eval/results/` and `eval/sim/results/` still carry the old tag, so a
/// reader that replays history must accept both.
pub const SCHEMA: &str = "gorp.trace/1";
...
fn session_id() -> Option<String> {
    std::env::var("GORP_SESSION_ID").ok().filter(|s| !s.is_empty())
}
...
fn binary() -> Value {
    json!({
        ...
        "git_sha": option_env!("GORP_GIT_SHA").unwrap_or(""),
        "git_dirty": option_env!("GORP_GIT_DIRTY").unwrap_or("") == "true",
        "profile": option_env!("GORP_BUILD_PROFILE").unwrap_or(""),
        "embed_dim": gorp_core::EMBED_DIM,
        ...
        "compat_key": gorp_core::cache::compat_key(),
    })
}

/// The cache knobs as the engine actually resolved them, not as the environment
/// spelled them. A scenario that pins `GORP_CACHE_TTL_SECS` and gets the
/// default anyway (the value latches in a `OnceLock` on first use) would
/// otherwise look like it had pinned it.
fn env_block() -> Value {
    json!({
        "cache_dir": gorp_core::cache::cache_base().to_string_lossy(),
        "cache_generation": gorp_core::cache::cache_generation().to_string_lossy(),
        "cache_max_bytes": gorp_core::cache::cache_max_bytes(),
        "ttl_secs": std::env::var("GORP_CACHE_TTL_SECS").ok(),
    })
}
...
fn path_taken(r: &gorp_core::search::SearchReport, mode: Mode) -> &'static str {
    ...
    merge(
        ...
        json!({
            ...
            "resolution": {
                ...
                "discover_calls": gorp_core::cache::discover_calls(),
            },
            "repair": r.repair,
            "timing": {
                "stages": r.stages,
...
pub fn index_envelope(
    root: &Path,
    opts: &gorp_core::store::BuildOptions,
    stats: &gorp_core::store::BuildStats,
    exit_code: i32,
) -> Value {
    merge(
        common(Phase::Index),
...
pub fn emit(envelope: &Value, stats_json: bool) {
    ...
    if let Ok(path) = std::env::var("GORP_TRACE_FILE")
        && !path.is_empty()
    {
