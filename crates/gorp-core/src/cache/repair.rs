...
//!
//! Validation is throttled by `GORP_CACHE_TTL_SECS` (default 60; 0 = always).
...
pub enum RepairOutcome {
    ...
    #[default]
    NotApplicable,
    /// The last validation was inside `GORP_CACHE_TTL_SECS`, so no drift
    ...
    /// the caller's back; a repo-local `.gorp/` keeps repairing however far
...
fn repair_ttl_secs() -> u64 {
    ...
    *TTL.get_or_init(|| {
        std::env::var("GORP_CACHE_TTL_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(60)
    })
}
...
/// for LRU. For a repo-local `.gorp/`, which is a committed artifact the
...
pub fn scope(
    ...
    // only do to something it owns. A repo-local `.gorp/` is the user's
