...
//! vector. Reads SIF stats from the corpus's live `.gorp/sif.bin`, so run
...
use gorp_core::corpus::doc_text;
use gorp_core::rank::normalize;
use gorp_core::text::{
...
fn unit(mut v: [f32; gorp_core::EMBED_DIM]) -> Vec<f32> {
    normalize(&mut v);
    v.to_vec()
}
