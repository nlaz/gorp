...
use gorp_core::trace::{Bucket, SCHEDULE_COLD, SCHEDULE_KEYWORD, SCHEDULE_WARM, Stage};
use gorp_core::{ChunkParams, rank::Mode, store};
use std::path::Path;
...
fn build_reports_its_own_schedule() {
    ...
    assert_eq!(
        stats.stages.names(),
        names(gorp_core::trace::SCHEDULE_BUILD),
